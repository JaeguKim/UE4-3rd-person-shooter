...
{
	// Prevent sprinting when moving backward
	float DotProduct = FVector::DotProduct(GetVelocity(), GetActorForwardVector());
	if (DotProduct <= 0)
		return;

	bWantsToRun = true;
	UCharacterMovementComponent * MoveComp = Cast<UCharacterMovementComponent>(GetComponentByClass(UCharacterMovementComponent::StaticClass()));
	float CurrentSpeed = MoveComp->GetMaxSpeed();
	MoveComp->MaxWalkSpeed = CurrentSpeed * 2;
}
